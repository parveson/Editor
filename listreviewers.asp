...
<HTML>
<HEAD>
<TITLE>List All Reviewers</TITLE>
<LINK rel="stylesheet" type="text/css" href="StyleSheet1.css">
<SCRIPT LANGUAGE="JavaScript" type="text/JavaScript">
function win(strPath)		
{
	window.open("" + strPath,"newWindowName","height=300,width=600,left=20,top=10,toolbar=no,location=0,directories=no,menubar=0,scrollbars=no,resizable=yes,status=yes");
}
...
Q = "SELECT * FROM [tblParty] WHERE [tblParty.Role]= 3 ORDER BY [tblParty.Lname]"
...
Response.Write("<th>Name</th><th>Email</th><th>Phone</th><th>Expertise</th><th>Notes</th><th>Delete?</th></tr>")
recordcount=0
For I = iRecFirst To iRecLast
	...
	For J = 0 To iFieldLast
		...
		case 7  ' Expertise 
			Response.Write("<td align=center class=small>" & arrDBData(J,I) & "</td>")
		case 8   ' Notes
			notes = left(arrDBData(J,I),20)  ' Truncate notes for a link
			Response.Write("<td align=center class=small>")
			%>
			<A href="javascript:win('reviewernotes.asp?id=<%=loginid%>&partyid=<%=partyid%>');"><%=notes%></A></td>	
			<%
		case else
...
%>
<br>
<p><b><a href="addreviewer.asp?id=<%=loginid%>">Add a reviewer</a></b></p>
<br>
